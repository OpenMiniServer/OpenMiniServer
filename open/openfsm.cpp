...
bool OpenFSM::isInState(int eState)
{
    if (!fsmState_)
    {
        return false;
    }
    return fsmState_->eState_ == eState;
}

bool OpenFSM::isInState(const std::string& stateName)
{
    if (!fsmState_)
    {
        return false;
    }
    return fsmState_->stateName_ == stateName;
}
...
OpenFSMState* OpenFSM::getState(int eState)
{
    ...
    auto state = OpenFSMPool_.getState(eState);
    if (!state)
    {
        assert(false);
        return 0;
...
OpenFSMState* OpenFSM::getState(const std::string& stateName)
{
    ...
    auto state = OpenFSMPool_.getState(stateName);
    if (!state)
    {
        assert(false);
        return 0;
