...
void OpenString::Split(const std::string& line, const std::string& split, std::vector<std::string>& vect_items)
{
    vect_items.clear();
...
std::string OpenString::ToString(double value)
{
    std::string ret = std::to_string(value);
    const char* ptr = strstr(ret.c_str(), ".");
    if (!ptr) return ret;
    while (ret.back() == '0') ret.pop_back();
    while (ret.length() - ret.find_last_of(".") - 1 > 3) ret.pop_back();
    if (ret.back() == '.') ret.pop_back();
    return ret;
}

std::string OpenString::ToString(int32_t value)
{
    return std::to_string(value);
}

std::string OpenString::ToString(int64_t value)
{
    return std::to_string(value);
}
...
std::string OpenString::UrlEncode(const std::string& str)
{
    std::string strTemp = "";
...
std::string OpenString::UrlDecode(const std::string& str)
{
    std::string strTemp = "";
    size_t length = str.length();
    unsigned char c;
    ...
    for (size_t i = 0; i < length; i++)
    {
        ...
        {
            high = hex2dec((unsigned char)str[++i]);
            low = hex2dec((unsigned char)str[++i]);
            strTemp += high * 16 + low;
        }
    }
    return strTemp;
...
void OpenString::MD5Hash(const std::string& input, std::string& output)
{
    ...
    md5_init(&state);
    md5_append(&state, (const md5_byte_t*)input.data(), (int)input.size());
    md5_finish(&state, digest);

    char hexOutput[(MD5_DIGEST_LENGTH << 1) + 1];
...
void OpenString::TrimComment(const std::string& input, std::string& output)
{
    size_t size = 0;
    std::string ret;
    std::string::size_type start_pos = 0;
    std::string::size_type end_pos   = input.find("\n");
    while(end_pos != std::string::npos)
    {
        size = end_pos - start_pos;
        if(size > 0)
        {
            ret = input.substr(start_pos, size);
            while (!ret.empty() && (ret[0] == ' ' || ret[0] == '\r' || ret[0] == '\n' || ret[0] == '\t')) ret.erase(ret.begin());
            if (!ret.empty() && ret[0] != '/')
            {
                output.append(ret);
            }
    ...
    if(start_pos < input.size())
    {
        ...
        if (!ret.empty()) output.append(ret);
    }
}

bool OpenString::GBKToUTF8(const std::string& strGBK, std::string& strUTF8)
{
    if (strGBK.empty())
    {
        return false;
    ...
    len = MultiByteToWideChar(CP_ACP, 0, strGBK.c_str(), -1, strUnicode, len);
    if (len < 1)
    {
        free(strUnicode);
        return false;
    ...
    strUTF8.resize(len);
    len = WideCharToMultiByte(CP_UTF8, 0, strUnicode, -1, (char*)strUTF8.data(), len, NULL, NULL);
    free(strUnicode);
    if (len < 1)
    {
        return false;
