...
private:
    ...
    public:
        ...
        template <class T>
        bool registerTheState(const std::string& stateName, int eState = -1)
        {
            std::map<std::string, OpenFSMState*>::iterator iter = mapNameState_.find(stateName);
            if (iter != mapNameState_.end())
            {
                printf("OpenFSMPool::registerTheState stateName [%s] is exist!\n", stateName.c_str());
                assert(false);
                return false;
            }
            T* state = new T;
            state->eState_ = eState;
            state->stateName_ = stateName;
            if (state->eState_ <= 0)
            {
                state->eState_ = -uid_++;
            }
...
public:
    ...
    template <class T>
    static bool RegisterTheState(const std::string& stateName, int eState = -1)
    {
        return OpenFSMPool_.registerTheState<T>(stateName, eState);
    }
